...
@Service("cookidooEmailServiceImpl")
@Slf4j
public class CookidooEmailServiceImpl implements EmailService {
    ...
    @SneakyThrows
    public CookidooEmailServiceImpl(@Value("${api.access.key.mailslurp}") String apiKey) {
    ...
    public String getEmail() {
        try {
            ...
            log.error("[CookidooEmailServiceImpl] {}", e.getResponseBody());
            throw new RuntimeException(e);
        }
        log.info("[CookidooEmailServiceImpl] indexId: {}", inbox.getId());

        String tempEmailAddress = inbox.getEmailAddress();
        log.info("[CookidooEmailServiceImpl] Temporary email address: {}", tempEmailAddress);
    ...
    public String getResponse() {
        ...
        try {
            ...
            log.error("[CookidooEmailServiceImpl] error when try to retriev emial with confirmation code {}", e.getResponseBody());
            throw new RuntimeException(e);
        }
        String confirmationCode = extractFinalConfirmationLink(email);
        log.info("[CookidooEmailServiceImpl] confirmation code: {}", confirmationCode);
